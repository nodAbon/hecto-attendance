/**
 * 15일 전부터 현재 시점까지의 세콤/캡스 출입기록을 가져와서 Supabase에 적재하는 일회성/수동 동기화 스크립트.
 * 파일명: index15days.js
 * 실행 방법: node index15days.js
 */
...
async function main() {
  ...
  try {
    ...
    log('INFO', `15일간의 동기화 완료 (${elapsed}s) | 세콤 ${secomCount}건 | 캡스 ${capsCount}건`);

  } catch (err) {
    log('ERROR', '15일간의 동기화 작업 실패', err.message);
    process.exitCode = 1;
  } finally {
